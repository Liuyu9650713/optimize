...
E=10; R=1000;k1=0.005;k2=5e-04;k3=3;k4=0.001;k5=0.03;k6=0.1;k7=7.2;k8=10;k9=0.9;P=90;
%E=1; R=1000;k1=0.005;k2=5e-04;k3=3;k4=0.001;k5=0.03;k6=0.1;k7=7.2;k8=10;k9=0.9;P=9;

N=5;
dt=0.1;tau1=1:dt:4;
tau=[4 0 4 4 4];%时间段theta(i)
C=[2 2 2 2];
...
for j=1:3/dt+1
    ...
    for i=1:N
        Tspan=[i-1:0.1:i];
        if i==2
             [tt,yy]=ode45(@state,Tspan,y0,[],tau1(j)); %solve state, tt is the column vector, yy is the matrix(column:S,I,P,V)
        else
            [tt,yy]=ode45(@state,Tspan,y0,[],tau(i));
        end
        ...
        %下面开始调用协态函数求解协态
         ...
         for i=1:N
            ...
            %i对应第N-i+1个时间段先求第5段，然后第4段，...
             ...
             if i==N-1
                 [inv_tti,inv_lamdai]=ode45(@(t,y)costate(t,y,ti,yi(:,1),yi(:,2),yi(:,3),yi(:,4),yi(:,5),tau1(j)),inv_tspan, inv_lamdai0);
             else
       ...
       for i=1:N
           ...
           if i==2
              h=stan_y5+stan_lam1.*(k1*E-(k2+k3)*stan_y1)+stan_lam2.*(k2*stan_y1-k5*stan_y2)+stan_lam3.*(k4*stan_y2*R-k6*stan_y5.*stan_y3)...
                 +stan_lam4.*(k6*stan_y5.*stan_y3-k7*stan_y4)+stan_lam5.*(k8-k9*stan_y5-k6*stan_y5.*stan_y3);
              grad_CC1=trapz(stan_t,h);
              grad_C1=[grad_C1 grad_CC1];
           end
       end
      ...
      figure(1)
      plot(tau1,obj_f1,'b','LineWidth',2)
      hold on
      figure(4)
      plot(tau1,grad_C1,'bo',tau1(1:3/dt),obj_fgrad,'ro','LineWidth',1)%红色的是差分
      hold on