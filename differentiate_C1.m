...
int_L=[];
    ...
    for i=1:N
        Tspan=[i-1:0.1:i];
        [tt,yy]=ode23s(@state,Tspan,y0,[],tau(i)); %solve state, tt is the column vector, yy is the matrix(column:S,I,P,V)
        ...
        switch i
            case 1
                y0=[C1(j)*E+yb(i,1)  yb(i,2)  yb(i,3)  yb(i,4)  yb(i,5)];
                case 2
                    y0=[C2*E+yb(i,1)  yb(i,2)  yb(i,3)  yb(i,4)  yb(i,5)];
                    
                case 3
                        y0=[C3*E+yb(i,1)  yb(i,2)  yb(i,3)  yb(i,4)  yb(i,5)];
                case 4
                         y0=[C4*E+yb(i,1)  yb(i,2)  yb(i,3)  yb(i,4)  yb(i,5)]; 
        ...
        %bb=sum(int_L);
        %bbb=[bbb bb];
        obj_f=P*E*C1(j)+P*E*C2+P*E*C3+P*E*C4+sum(int_L);%成本，数
        obj_f1=[obj_f1 obj_f];
      ...
      figure(1)
      plot(C1,obj_f1,'b','LineWidth',2)
      hold on
      figure(4)
      plot(C1,grad_C1,'ko',C1(1:5/dt),obj_fgrad,'ro','LineWidth',1)%红色的是差分
      hold on
